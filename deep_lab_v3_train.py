...
from deep_lab_v3_utils import init, read_config_file, get_tf_dataset
import deep_lab_v3_model

os.environ['TF_CPP_MIN_LOG_LEVEL'] = '3'
param_config_file_name = os.path.join(os.getcwd(), 'deep_lab_v3_config.json')
...
def batch_train(FLAGS):
    ...
    model_dir = FLAGS.model_dir + \
        str(FLAGS.output_strides) + '_' + str(FLAGS.num_epochs)
    init(model_dir)
    ...
    net_arch = deep_lab_v3_model.DeepLab3(
        FLAGS.pretrained_weights, is_training, FLAGS.data_format, FLAGS.num_classes, FLAGS.output_strides)
    net_arch.resnet50_encoder(features)
    print('resnet-50 encoder built')
    net_arch.deeplabv3()
    print('deeplabv3 built with strides : ' + str(FLAGS.output_strides))
...
def main():
    ...
    output_strides = config['model']['output_strides']
    ...
    parser.add_argument('-output_strides', default=output_strides,
                        type=int, choices=[8, 16], help='output strides for model to use')
