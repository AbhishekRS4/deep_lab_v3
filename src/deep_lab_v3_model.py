...
class DeepLab3:
    def __init__(self, pretrained_weights, is_training, data_format="channels_first", num_classes=15, output_strides=16):
        ...
        if self._data_format == "channels_first":
            ...
            if self._output_strides == 16:
                self._encoder_dilations = [1, 1, 2, 2]
            else:
                self._encoder_dilations = [1, 1, 4, 4]
        else:
            ...
            if self._output_strides == 16:
                self._encoder_dilations = [1, 2, 2, 1]
            else:
                self._encoder_dilations = [1, 4, 4, 1]
    ...
    def resnet50_encoder(self, features):
        ...
        self.stage0 = self._get_relu_activation(self.stage0, name="relu1")
        ...
        if self._output_strides == 16:
            ...
            self.stage4 = self._res_conv_block(
                input_layer=self.stage3, stage="4a", strides=self._encoder_conv_strides)
        ...
        # Stage 5
        self.stage5 = self._res_conv_block(input_layer=self.stage4, stage="5a",
            strides=self._encoder_conv_strides, dilations=self._encoder_dilations)
    ...
    def deeplabv3(self):
        ...
        self.decoder_conv = self._get_conv2d_layer(self.aspp_out, self._num_classes,
            [1, 1], [1, 1], name="decoder_conv")
        self.logits = self._get_upsample_layer(
            self.decoder_conv, self._input_size, name="upsample" + str(self._output_strides))
    ...
    def _atrous_spatial_pyramid_pool_block(self, input_layer, depth=256, name="aspp_"):
        ...
        _conv1x1 = self._get_conv2d_layer(input_layer, depth, [1, 1], [1, 1],
            name=name + "conv1x1")
        ...
        _conv3x3_1 = self._get_conv2d_layer(input_layer, depth, [3, 3], [1, 1],
            dilation_rate=k * self._atrous_rate[0], name=name + "conv3x3_1")
        ...
        _conv3x3_2 = self._get_conv2d_layer(input_layer, depth, [3, 3], [1, 1],
            dilation_rate=k * self._atrous_rate[1], name=name + "conv3x3_2")
        ...
        _conv3x3_3 = self._get_conv2d_layer(input_layer, depth, [3, 3], [1, 1],
            dilation_rate=k * self._atrous_rate[2], name=name + "conv3x3_3")
    ...
    def _res_conv_block(self, input_layer, stage, strides, dilations=[1, 1, 1, 1]):
        x = self._res_conv_layer(input_layer,
            name="res" + stage + "_branch2a", strides=strides, dilations=dilations)
        x = self._get_batchnorm_layer(x, name="bn" + stage + "_branch2a")
        x = self._get_relu_activation(x, name="relu" + stage + "_branch2a")
        ...
        x = self._get_relu_activation(x, name="relu" + stage + "_branch2b")
        ...
        shortcut = self._res_conv_layer(input_layer,
            name="res" + stage + "_branch1", strides=strides, dilations=dilations)
        shortcut = self._get_batchnorm_layer(shortcut, name="bn" + stage + "_branch1")

        x = tf.add(x, shortcut, name="add" + stage)
        x = self._get_relu_activation(x, name="relu" + stage)
