...
class DeepLab3:
    ...
    def deeplabv3(self):
        ...
        self.decoder_conv = self._get_conv2d_layer(
            self.aspp_out, self._num_classes, [1, 1], [1, 1], name="decoder_conv")
    ...
    def _get_batchnorm_layer(self, input_layer, name="bn"):
        bn_layer = tf.layers.batch_normalization(input_layer, axis=self._feature_map_axis,
    ...
    def _res_batchnorm_layer(self, input_layer, name):
        ...
        bn = tf.nn.batch_normalization(input_layer, mean=mean, variance=std, offset=beta,
            scale=gamma, variance_epsilon=1e-12, name=name)
